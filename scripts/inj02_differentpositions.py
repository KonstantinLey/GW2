...
time_offset = np.arange(10)*100.

GW170817_injections['time'] = Start_time+time_offset

GW170817_injections['theta_jn'] = np.ones(Ninj)*2.79240799361
...
realeventra = 3.44613122778 + 2*np.pi*(Start_time-event_time)/time_sid_day
realeventdec = -0.406661715715

#events with epsilon_D
event1ra = -2.63843467103
event1dec = 0.72171723123
event2ra = -1.07235545032
event2dec = 0.435546128651
event3ra = -0.669829064279
event3dec = 0.190256612154
event4ra = -0.185539506068
event4dec = -0.935559373817

#events calculated with xi at time(GW170817), therefore they have to be corrected
event5ra =  0.0317332591272 + 2*np.pi*(Start_time-event_time)/time_sid_day
event5dec = -1.15826395814
event6ra =  2.88772658057 + 2*np.pi*(Start_time-event_time)/time_sid_day
event6dec = 1.06306418076
event7ra = 2.76079354406 + 2*np.pi*(Start_time-event_time)/time_sid_day
event7dec = 0.872664625997
event8ra = 2.5703939893 + 2*np.pi*(Start_time-event_time)/time_sid_day
event8dec = 0.904397885124

#Minimize error of Fisher matrix
event9ra = 1.50271190121
event9dec = 1.28519699465


ra = np.array([realeventra,event1ra,event2ra,event3ra,event4ra,event5ra,event6ra,event7ra,event8ra,event9ra])
dec = np.array([realeventdec,event1dec,event2dec,event3dec,event4dec,event5dec,event6dec,event7dec,event8dec,event9dec])

GW170817_injections['ra'] = ra + 2*np.pi*time_offset/(time_sid_day)
GW170817_injections['dec'] = dec
